...
public class PaintableObject
{
	...
	public static final String[] splitStringMultiline(String var0, int var1, BitmapFont var2)
	{
		...
		while(true)
		{
			...
			while(true)
			{
				...
				layout.setText(var2, var7);
				if(layout.width > var1)
				{
					if(var11 == var4)
					{
	...
	{
		// Strip leading slash if present for Gdx.files.internal
		if (var0.startsWith("/")) {
			var0 = var0.substring(1);
		}
		
		DataInputStream dis = new DataInputStream(Gdx.files.internal(var0).read());
...
public void readLocale(DataInputStream dis) throws IOException
{
	...
	{
		if(localeStrings != null && var0 < localeStrings.length)
		{
