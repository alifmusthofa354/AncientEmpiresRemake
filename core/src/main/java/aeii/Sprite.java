...
public class Sprite
{
	...
	private static void initWhitePixel() {
		if (whitePixel == null) {
	...
	public final void paintFrame(SpriteBatch batch, int var2, int var3, int var4, int var5)
	{
		if(mode != 2 && mode != 4 && mode != 3)
		{
			if(visible)
			{
				...
				frames[var2].paintEx(batch, var6, var7, var5);
			}

		}
		else
		{
			paint(batch, var3, var4);
		}
	}

	public final void paintCurrentFrame(SpriteBatch batch, int var2, int var3, int var4)
	{
		paintFrame(batch, currentFrameSequence[currentFrame], var2, var3, var4);
	}
	
	private void setColor(SpriteBatch batch, int color) {
	...
	private void drawLine(SpriteBatch batch, int x1, int y1, int x2, int y2) {
		float dx = x2 - x1;
		float dy = y2 - y1;
		float dist = (float)Math.sqrt(dx * dx + dy * dy);
		float angle = (float)Math.toDegrees(Math.atan2(dy, dx));
		batch.draw(whitePixel, x1, y1, 0, 0, dist, 1, 1, 1, angle);
	}
	
	private void fillRect(SpriteBatch batch, int x, int y, int w, int h) {
		batch.draw(whitePixel, x, y, w, h);
	}
	
	private void fillArc(SpriteBatch batch, int x, int y, int w, int h) {
		// Approximation with rect
		batch.draw(whitePixel, x, y, w, h);
	}

	public void paint(SpriteBatch g, int offsetX, int offsetY)
	{
		int x;
		int y;
		...
		if(mode != 2 && mode != 4)
		{
			if(mode == 6)
			{
				...
				{
					y = currentX + 15;
					fillArc(g, currentX, currentY - 15, 30, 30);
					fillRect(g, y, currentY - 15, Renderer.width - y, 30);
					g.setColor(Color.WHITE);
					return;
				}

				fillArc(g, currentX - 30, currentY - 15, 30, 30);
				fillRect(g, 0, currentY - 15, currentX - 15, 30);
				g.setColor(Color.WHITE);
			...
			if(mode == 3)
			{
				...
				if(shiftX > 0)
				{
					drawLine(g, currentX, currentY, currentX + 4, currentY - 2);
					g.setColor(Color.WHITE);
					return;
				}

				drawLine(g, currentX - 4, currentY - 2, currentX, currentY);
				g.setColor(Color.WHITE);
				return;
			}
		...
		else
		{
			...
			for(int i = 0; i < 5; ++i)
			{
				if(rectPaintFlags[i])
				{
					...
					fillRect(g, x, y, rectSize[i], rectSize[i]);
				}
			}
			g.setColor(Color.WHITE);
		}
